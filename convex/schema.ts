...
export default defineSchema(
  {
    revenue_ingestion_receipts: defineTable({collectorRunId:v.string(),startedAt:v.number(),receivedAt:v.number(),
      updatedAt:v.number(),status:v.union(v.literal('collecting'),v.literal('processing'),v.literal('verified'),v.literal('rejected')),
      origin:v.optional(v.union(v.literal('manual'),v.literal('scheduled'))),
      code:v.optional(v.string()),retryable:v.optional(v.boolean())})
      .index('by_run',['collectorRunId']).index('by_started',['startedAt']).index('by_origin_started',['origin','startedAt']),
